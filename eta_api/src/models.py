...
# Define database models (tables and their relationships)
...
class ETAQuery():
    ...
    def get_speed_at_point(self, lat, lon):
        return 25  # TODO: Take speed from route model after it is precomputed
    ...
    def generate_eta(self):
        ...
        for future_checkpoint in range(next_closest_checkpoint + 1, n):
            ...
            # Unit conversion is to put meters into kilometers and time into minutes
            eta_next_to_future = (
                    (self.route_model.iloc[b]['trip(m)'] - self.route_model.iloc[a]['trip(m)']) / 1000) / self.speed * 60
